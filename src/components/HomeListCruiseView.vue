<template>
  <div class="flex justify-center mt-48 relative rounded-3xl">
    <div class="min-h-screen w-full max-w-screen-xl">
      <div class="head grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
      ...
      <div class="grid grid-cols-3 ">
        <Card class="max-w-md rounded-4xl shadow-1 m-3 p-3" v-for="cruise in cruises" :key="cruise.id">
          <template #header>
            <div class="relative">
              <img :alt="cruise.name" :src="cruise.image" class="w-full h-60 object-cover rounded-3xl pb-2"/>
              <div class="absolute top-3 left-3 bg-yellow-300 text-orange-900 px-3 py-1 rounded-2xl text-sm flex items-center gap-1 opacity-85">
                <i class="pi pi-star"></i> 4.5 (2) đánh giá
              </div>
              <div class="flex gap-1 h-0.5rem bg-gray-100 rounded-full px-3 py-1 w-fit left-3 absolute">
          ...
          <template #title>
            <FloatLabel class="lg:font-bold text-lg pt-3 whitespace-nowrap overflow-hidden text-ellipsis">
              {{ cruise.name }}
            </FloatLabel>
          </template>
          <template #content>
            <p class="font-medium text-gray-700  align-items-center">
          ...
          <template #footer>
            <div class="flex justify-between align-items-center w-full">
              ...
              <Button class="inline-flex items-center px-5 py-2 text-sm font-medium text-center" label="Đặt ngay"/>
            </div>
          </template>
        </Card>
      </div>
